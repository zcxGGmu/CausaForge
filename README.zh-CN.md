...
```text
阅读 README.zh-CN.md 和 ROADMAP.md。使用 `bun install --ignore-scripts` 和 `bun run build` 构建 CausaForge。使用 `bun run test`、`bun run typecheck` 和 `bun run build` 验证。然后把 `dist/index.js` 注册为 file:// OpenCode plugin。先调用 workflow_start；当 Agent3 metadata 要求源码准备时调用 workflow_prepare_repository；每次 transition 前记录当前阶段所需 artifact；building 阶段捕获 implementation diff；只能在 delivering 阶段通过 workflow_complete 关闭 workflow。
```
...
| 十一个 workflow tools | 导入 Agent3 蓝图文件夹、启动 workflow、准备源码仓库、记录 artifact、校验 artifact、捕获 diff、执行受控验证 manifest、推进 phase、回滚 phase、报告状态并完成交付 | `packages/causaforge-opencode/src/tools/index.ts` |
